...
projection), `verdict` is exactly `start` or `sit`, `why` names the numbers
you used (projection, injury/practice status, opponent), and `confidence`
...
number to fill the field. `proj` on a starter/bench/opponent row is a
plain number, never null - use Sleeper's own projection, or `0` only when
Sleeper genuinely has none for that player. Also set `team_record`,
`opponent`, `opponent_record`, `my_proj`, `opp_proj`, and a one-line
`summary`. Cover the bench and, if the roster carries any, reserve/IR
players too (`bench`/`reserve`, each `{player, proj, why}`, reserve omits
`proj`), each with a one-line `why` even when the call is "no change," plus
the opponent's own starters (`opponent_starters`).
