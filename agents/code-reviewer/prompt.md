...
CI status is part of the verdict. The envelope's `<checks>` section (untruncated: `check-runs.json` in the context dir) is the PR's check status at dispatch time. A failing check means no `approve`: if the diff plausibly causes it, that's a 🚨 **blocking:** finding naming the check; if it's clearly unrelated or pre-existing, say so in the summary and use `comment` - either way the failure appears in your output, never silently approved past. Pending or queued checks don't block; note them in the summary so the merger knows CI hadn't finished.
