...
func TestForgetSweep_DefaultRules(t *testing.T) {
	...
	seedMemory(t, s, point{ID: "verified-old", Content: "verified, no recalls in a year", Scope: "repo:r",
	...
	seedMemory(t, s, point{ID: "unverified-stale", Content: "unverified, never upvoted, 91 days old", Scope: "repo:r",
		Author: "a", Timestamp: "t", MintedAt: daysAgo(91), ValidFrom: "t",
		Status: string(StatusUnverified), Tier: TierUnverified})
	seedMemory(t, s, point{ID: "bad-score", Content: "downvoted into oblivion", Scope: "repo:r",
	...
	seedMemory(t, s, point{ID: "fresh-unverified", Content: "unverified but only 3 days old", Scope: "repo:r",
	...
	assertStatus(t, s, "verified-old", string(StatusReinforced))
	assertStatus(t, s, "unverified-stale", string(StatusUnverified))
	assertStatus(t, s, "bad-score", string(StatusUnverified))

	// Real sweep applies the same matches.
	if _, err := s.ForgetSweep(ctx, false); err != nil {
		t.Fatalf("real sweep: %v", err)
	}
	assertStatus(t, s, "verified-old", string(StatusReinforced)) // kept
	assertStatus(t, s, "unverified-stale", string(StatusInvalidated))
	assertStatus(t, s, "bad-score", string(StatusInvalidated))
	assertStatus(t, s, "fresh-unverified", string(StatusUnverified)) // untouched
...
func TestSweepOnce_ForgetThenRetain(t *testing.T) {
	...
	seedMemory(t, s, point{ID: "ages-out", Content: "c", Scope: "repo:r", Author: "a", Timestamp: "t",
