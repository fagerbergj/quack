...
// same dir must not re-run it. Shared across every RunCheckSetup caller (clone/worktree provisioning and the gate's own checks), keyed by the jail-resolved absolute path, so a dir bootstrapped at provisioning time is a cache hit (no-op) when the gate later asks for the same dir.
...
// worktree linked off the shared clone (workers can't bootstrap a read-only tree by definition). A failing step logs Warn and stops: a broken bootstrap must not become a new way to fail a node or a provisioning call.
