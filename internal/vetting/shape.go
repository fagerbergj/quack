...
// exactly like mermaid.go's mermaidCriterion: no model judgement involved, a hard weakest-link fail when they fire. "The deliverable isn't a
// deliverable" slips past a judge that only scores content quality, never whether the text is even a well-formed answer.
package vetting
...
// toolCallSyntaxMarkers are fragments of tool-call wire syntax that can never legitimately appear in a delivered answer: a model that emitted a malformed
// or unparseable call sometimes leaks the raw fragment into its text response instead (#565 - the NightsOut#57 comment ended in a bare
// `</parameter></function></tool_call>`). Detecting the closing tags is enough - by the time one appears, the syntax has already leaked.
...
// tool-call syntax. ok=false means nothing was found - a clean answer needs no entry, matching mermaidCriterion's convention.
