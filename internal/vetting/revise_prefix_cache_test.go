// revise_prefix_cache_test.go: regression test for finding 3 of the agent-efficiency
// audit (stable-first section ordering in buildRevisionContent/buildFinalizeContent/
// buildContinuationPrompt). Prod serves workers and judge from the same vLLM model
// with prefix caching, so a byte that is stable across rounds and sits before the
// first changing byte is a prefill saved. This pins a MINIMUM shared-prefix fraction
// between revise round 1 and round 2 on a synthetic-but-realistic task, so a future
// reorder that kills the cache fails loudly.
package vetting
...
// per-round evidence, so the shared-prefix fraction reflects section order,
...
// revise round 2 (a different verdict, same original question/answer) must
// share at least 70% of revise round 1's bytes as a common prefix - the
// fraction the reorder measured at 80%+ on the audit's synthetic task
// (24,135 of 29,101 bytes), comfortably above the pre-fix 3.3%.
...
// question, the instruction paragraph, the verdict, and the previous answer
// - is still present, just reordered stable-first.
