...
Read the REQUEST TYPE carefully before judging criterion 2 — the most common mistake is forcing a code-implementer onto a plan-only or research-only request just because its acceptance text happens to mention "PR" or "commit" (e.g. an issue whose body describes what the EVENTUAL implementation PR should look like, while THIS request only asks for the plan). Judge what THIS request is actually asking the plan to DO, not every verb it contains.

Give the plan the benefit of the doubt on ambiguous phrasing — this check exists to catch a plan that is CLEARLY the wrong shape for the request (implement-and-deliver routed to research-only, or vice versa), not to nitpick a reasonable decomposition.

Call submit_plan_verdict exactly once with accept (bool) and reason (if rejecting: name the ONE specific edit that fixes it — e.g. "add a terminal code-implementer node" or "this is a plan-only request; remove the code-implementer node" — never a vague "reconsider the plan"; if accepting: a brief reason is fine, or "").`
