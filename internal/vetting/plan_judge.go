...
Give the plan the benefit of the doubt on ambiguous phrasing — this check exists to catch a plan that is CLEARLY the wrong shape for the request (implement-and-deliver routed to research-only, or vice versa; a monolithic implementer where the feature obviously decomposes into several independent portions; or a single cohesive change pointlessly split into activity-scoped nodes), not to nitpick a reasonable decomposition.

Call submit_plan_verdict exactly once with accept (bool) and reason (if rejecting: name the ONE specific edit that fixes it — e.g. "add a terminal code-implementer node", "this is a plan-only request; remove the code-implementer node", "set delivery.kind to pull_request and setup.repo/work_branch to the repo URL and a branch name", or "split the implementer into a chain of independent, goal-scoped nodes" — never a vague "reconsider the plan"; if accepting: a brief reason is fine, or "").`
