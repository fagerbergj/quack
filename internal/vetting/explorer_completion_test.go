...
func TestReadOnlyNodeIsNotHeldToTheUserRequestsDelivery(t *testing.T) {
	...
	if workIncomplete(answer, explorerTask, act) {
	...
	if !workIncomplete(answer, assembledPrompt, act) {
...
func TestImplementerIsStillHeldToItsDelivery(t *testing.T) {
	...
	if !workIncomplete(answer, implementerTask, workerActivity{}) {
		t.Fatal("an implementer that committed and pushed NOTHING was accepted as finished")
	}
	if !workIncomplete(answer, implementerTask, workerActivity{committed: true}) {
