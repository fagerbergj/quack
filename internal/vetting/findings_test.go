...
func TestJudgeFindings_ContradictedSinksGroundingCriterion(t *testing.T) {
	...
	v, err := runJudgeAgent(t.Context(), factory, cfg, q, "LGTM.", act, nil, nil, func(*genai.Part) bool { return true })
	if err != nil {
...
func TestJudgeFindings_ContextDependentRefutationReachesRelatedFile(t *testing.T) {
	...
	v, err := runJudgeAgent(t.Context(), factory, cfg, q, "See findings.", act, nil, nil, func(*genai.Part) bool { return true })
	if err != nil {
		t.Fatalf("runJudgeAgent: %v", err)
	}
