...
// TestGetTurnsWithContent_QueuedTurnShowsPersistedInput pins the queued-chat
// bug: a run stuck behind the worker semaphore has a ChatTurn row but no
// session events yet, so the old session-walk-only userText was always "".
// SetTurnInput must make the trigger text available immediately.
func TestGetTurnsWithContent_QueuedTurnShowsPersistedInput(t *testing.T) {
	st, err := New("sqlite", filepath.Join(t.TempDir(), "quack.db"))
	if err != nil {
		t.Fatalf("New sqlite: %v", err)
	}
	...
	if err := st.SetTurnInput(ctx, c.ID, "t1", "fix the flaky test"); err != nil {
		t.Fatalf("SetTurnInput: %v", err)
	}
	// No session events at all - the run hasn't started, exactly the queued state.

	turns, err := st.GetTurnsWithContent(ctx, chatAppName, "local", c.ID)
	if err != nil || len(turns) != 1 {
		t.Fatalf("GetTurnsWithContent: %+v err=%v", turns, err)
	}
	if got := turns[0].UserText; got != "fix the flaky test" {
		t.Errorf("UserText = %q, want the persisted input (no session events exist yet to fall back to)", got)
	}
}

// TestGetTurnsWithContent_InputFallbackForPreExistingRows proves a turn saved
// before this column existed (no SetTurnInput call) still gets its text once
// the run's session events land - same asymmetry-avoiding fallback as usage.
func TestGetTurnsWithContent_InputFallbackForPreExistingRows(t *testing.T) {
	st, err := New("sqlite", filepath.Join(t.TempDir(), "quack.db"))
	if err != nil {
		t.Fatalf("New sqlite: %v", err)
	}
	...
	// SaveTurn only - no SetTurnInput, mirroring a pre-migration row.
	if err := st.SaveTurn(ctx, c.ID, "t1"); err != nil {
		t.Fatalf("SaveTurn: %v", err)
	}
	...
	userEv := session.NewEvent(ctx, "test")
	userEv.Author = "user"
	userEv.Content = &genai.Content{Role: "user", Parts: []*genai.Part{{Text: "hi from the session walk"}}}
	if err := st.Sessions.AppendEvent(ctx, sessResp.Session, userEv); err != nil {
		t.Fatalf("AppendEvent user: %v", err)
	}
	...
	if got := turns[0].UserText; got != "hi from the session walk" {
		t.Errorf("UserText = %q, want the session-walk fallback (no persisted input on this row)", got)
	}
}
