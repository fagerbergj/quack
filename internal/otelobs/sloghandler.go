...
// passthrough to next, so existing log output is byte-for-byte unchanged for any call site not using a *Context slog variant with a spanned ctx (e.g. slog.Info, or slog.InfoContext(context.Background(), ...)).
