...
// `replay` command, which resolves the bundle, synthesizes the config, and owns InProcessFromConfig/stop; this function only ever talks HTTP, same as every other bulletproof-CLI entry point in this package): create a fresh chat, send prompt (the bundle's recorded user turn), and stream the run the same way `chat show -f` does, applying the same pause/failure exit-code semantics as `-p`/`chat send` (see Report). Returns the process exit code.
