...
// EmitServerConfig turns it into a quack.yaml. Kept here, not in the wizard package, so it's testable without constructing a Huh form. The yaml tags are also what `server init --answers <file>` unmarshals into.
...
// caller prints the matching export commands. Non-secrets (endpoint, model names) are hardcoded - the wizard output works with no env setup beyond the one secret. The agent roster is tailored: media-reader only with an audio model, image-reader only with a vision model.
...
// endpoint and model names are hardcoded into the config, so there's nothing to export beyond the key.
...
// field's placeholder in the wizard, so the default tracks the selected kind. Kinds that need no URL (exa is API-key based; direct fetches in-process) return "".
