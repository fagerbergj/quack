...
// FinishRun is the one correct sequence for ending a run: flush, THEN cancel, THEN
// unregister+close atomically. Unregister must precede the close that lets a client's
// stream return, or a cancel racing "stream ended" still finds the run registered (TestSendChatMessage_ResponseCreatedFirst); doing both under the hub lock keeps a new dispatch from being closed by the old run's tail.
