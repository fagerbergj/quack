	...
	drain:
		...
		if len(batch) > 0 {
			if err := l.store.InsertChatEvents(context.Background(), batch); err != nil {
				slog.Warn("event log: persist failed; dropping batch", "component", "eventlog", "n", len(batch), "err", err)
			} else {
				// Trim once per chat per batch, to the max seq seen for that chat here.
