...
func TestWriteArtifact_SchemaValid_Succeeds(t *testing.T) {
	...
	rt := tl.(runnableTool)
	out, err := rt.Run(newArtifactsToolCtx(), map[string]any{"kind": "text", "mime": "application/json", "bytes": `{"name":"trade idea"}`})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestWriteArtifact_SchemaViolation_RefusesWithExactText(t *testing.T) {
	...
	rt := tl.(runnableTool)
	_, err = rt.Run(newArtifactsToolCtx(), map[string]any{"kind": "text", "mime": "application/json", "bytes": `{"other":1}`})
	if err == nil {
...
func TestWriteArtifact_InvalidJSON_Refuses(t *testing.T) {
	...
	rt := tl.(runnableTool)
	_, err = rt.Run(newArtifactsToolCtx(), map[string]any{"kind": "text", "mime": "application/json", "bytes": "not json"})
	if err == nil {
...
func TestWriteArtifact_UnregisteredKindUnaffected(t *testing.T) {
	// Registry only knows about "text" - "bytes" has no declared schema, so
...
func TestEditArtifact_SchemaViolation_RefusesAndPriorRevisionIntact(t *testing.T) {
	rc := recordstore.New(artifact.InMemoryService(), "quack", "u1", "chat-a").WithSchemas(nameSchemaRegistry(t, "text"))
	id, rev, err := rc.SaveBlob(context.Background(), "text", []byte(`{"name":"x"}`), "application/json", "", recordstore.Lineage{NodeID: "n1"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(err.Error(), `kind "text" failed its schema`) {
