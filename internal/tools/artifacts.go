...
func NewEditArtifactTool(c *recordstore.Client, nodeID string) (tool.Tool, error) {
...
func NewWriteArtifactTool(c *recordstore.Client, nodeID string) (tool.Tool, error) {
...
func NewWriteKindTool(c *recordstore.Client, nodeID, kind string, spec recordstore.KindSpec) (tool.Tool, error) {
...
func NewWriteKindTools(c *recordstore.Client, nodeID string) []tool.Tool {
	var out []tool.Tool
	for _, spec := range recordstore.Kinds() {
		t, err := NewWriteKindTool(c, nodeID, spec.Name(), spec)
		if err != nil {
			continue
		}
		out = append(out, t)
