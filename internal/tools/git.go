...
func pushForce(ctx context.Context, dir, branch string, caps workspace.Caps, auth *gitAuth) error {
	_, _, err := runGit(ctx, dir, []string{"push", "--quiet", "--force", "origin", branch}, caps, auth)
	return err
}
...
func rebaseOntoRemote(ctx context.Context, dir, branch string, caps workspace.Caps, auth *gitAuth) error {
	if _, _, err := runGit(ctx, dir, []string{"fetch", "--quiet", "origin", branch}, caps, auth); err != nil {
		return err
	}
	if _, _, err := runGit(ctx, dir, []string{"rebase", "FETCH_HEAD"}, caps, nil); err != nil {
		_, _, _ = runGit(ctx, dir, []string{"rebase", "--abort"}, caps, nil)
		return err
	}
	return nil
}
