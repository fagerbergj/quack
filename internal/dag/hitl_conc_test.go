...
func TestSpike_ConcurrentRunNode(t *testing.T) {
	...
	gn1 := newGatedNode(plan, plan.Nodes[0], mk(), nil, vetting.NewJudgeFactory(stub, nil, nil), cfg("w"), nil, nil, "", nil)
	gn2 := newGatedNode(plan, plan.Nodes[1], mk(), nil, vetting.NewJudgeFactory(stub, nil, nil), cfg("w"), nil, nil, "", nil)
