...
func runSingleNode(t *testing.T, plan Plan, cfg vetting.Config, stub model.LLM, refresh func(context.Context, Node, vetting.Config) bool) {
	...
	gateNodes := map[string]workflow.Node{
		plan.Nodes[0].ID: newGatedNode(plan, plan.Nodes[0], wn, nil, nil, nil, vetting.NewJudgeFactory(stub, nil, nil), cfg, nil, nil, "", nil, nil, nil, AdmissionSpec{}, AdmissionSpec{}, refresh, false),
	}
