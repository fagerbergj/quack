...
func TestJudgePhaseFreesWorkerSlotForSecondNode(t *testing.T) {
	...
	gn1 := newGatedNode(plan, plan.Nodes[0], wn1, nil, nil, nil, vetting.NewJudgeFactory(blocking, nil, nil), cfg, nil, nil, "", nil, nil, admission, workerSpec, judgeSpec, nil, false)
	...
	gn2 := newGatedNode(plan, plan.Nodes[1], wn2, nil, nil, nil, vetting.NewJudgeFactory(sig, nil, nil), cfg, nil, nil, "", nil, nil, admission, workerSpec, AdmissionSpec{}, nil, false)
...
func TestJudgePhasePauseReleasesExactlyOnce(t *testing.T) {
	...
	ag1, _ := llmagent.New(llmagent.Config{Name: "w1", Model: blocking, Description: "w", Instruction: "ROLE:w Answer."})
	wn1, _ := vetting.NewWorkerNode(ag1)
	gn1 := newGatedNode(plan, plan.Nodes[0], wn1, nil, nil, nil, vetting.NewJudgeFactory(blocking, nil, nil), cfg, nil, controls, chatID, nil, nil, admission, workerSpec, judgeSpec, nil, false)
	orchestrate := workflow.NewDynamicNode[any, string]("orch",
		func(ctx adkagent.Context, _ any, _ func(*session.Event) error) (string, error) {
			var wg sync.WaitGroup
			wg.Add(1)
...
func TestJudgePhaseNoDeadlockOnSimultaneousTransition(t *testing.T) {
	...
	for i := range nodes {
		...
		gates[i] = newGatedNode(plan, nodes[i], wn, nil, nil, nil, vetting.NewJudgeFactory(stub, nil, nil), cfg, nil, nil, "", nil, nil, admission, workerSpec, judgeSpec, nil, false)
	}
...
func TestSetupAdmissionReQueuesOnMidRunWait(t *testing.T) {
	...
	go func() {
		defer close(done)
		free, err := setupAdmission(ctx, "n1", cfg, admission, spec, AdmissionSpec{}, false)
		if err != nil {
