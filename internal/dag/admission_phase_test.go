...
func TestJudgePhaseFreesWorkerSlotForSecondNode(t *testing.T) {
	...
	gn1 := newGatedNode(plan, plan.Nodes[0], wn1, nil, nil, nil, vetting.NewJudgeFactory(nil, blocking, nil, nil), cfg, nil, nil, "", nil, nil, admission, workerSpec, judgeSpec, nil, nil)
	...
	gn2 := newGatedNode(plan, plan.Nodes[1], wn2, nil, nil, nil, vetting.NewJudgeFactory(nil, sig, nil, nil), cfg, nil, nil, "", nil, nil, admission, workerSpec, AdmissionSpec{}, nil, nil)
...
func TestJudgePhasePauseReleasesExactlyOnce(t *testing.T) {
	...
	ag1, _ := llmagent.New(llmagent.Config{Name: "w1", Model: blocking, Description: "w", Instruction: "ROLE:w Answer."})
	wn1, _ := vetting.NewWorkerNode(ag1)
	gn1 := newGatedNode(plan, plan.Nodes[0], wn1, nil, nil, nil, vetting.NewJudgeFactory(nil, blocking, nil, nil), cfg, nil, controls, chatID, nil, nil, admission, workerSpec, judgeSpec, nil, nil)
...
func TestJudgePhaseNoDeadlockOnSimultaneousTransition(t *testing.T) {
	...
	for i := range nodes {
		...
		gates[i] = newGatedNode(plan, nodes[i], wn, nil, nil, nil, vetting.NewJudgeFactory(nil, stub, nil, nil), cfg, nil, nil, "", nil, nil, admission, workerSpec, judgeSpec, nil, nil)
	}
