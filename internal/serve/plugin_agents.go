...
// mounting the module itself, shared here for plugin agent/shape gating.
...
// ResolveConfiguredPlugins resolves and admits every plugins.seed row
// against cfg with no session store or server startup - `quack server
// validate` uses it to preview what a real boot would seed.
func ResolveConfiguredPlugins(ctx context.Context, cfg *config.Config) ([]plugin.Plugin, error) {
	b := &boot{cfg: cfg}
	reg, rows, err := b.bootPluginRegistry(ctx, nil)
	if err != nil {
		return nil, err
	}
	...
	plugins, _, err = admitPlugins(ctx, reg, rows, plugins, cfg.Plugins.Seed, cfg.Extensions.Modules)
	return plugins, err
}
