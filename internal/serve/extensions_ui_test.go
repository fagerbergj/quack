...
func (fakeUIExtension) UI() extsdk.UIDescriptor {
	return extsdk.UIDescriptor{Title: "Fake UI", Href: "/fake-ui-test/home"}
}
...
func TestBuildSDKExtensions_UIDescriptorCaptured(t *testing.T) {
	...
	if sdkExts[0].title != "Fake UI" || sdkExts[0].href != "/fake-ui-test/home" {
		t.Errorf("title/href = %q/%q, want the UI() descriptor's values", sdkExts[0].title, sdkExts[0].href)
	}
	...
	want := schema.ExtensionInfo{Name: "fake-ui-test", Title: strPtrForTest("Fake UI"), Href: strPtrForTest("/fake-ui-test/home")}
	if descs[0].Name != want.Name || *descs[0].Title != *want.Title || *descs[0].Href != *want.Href {
...
func TestBuildSDKExtensions_NoUIDescriptor_NameOnly(t *testing.T) {
	...
	if len(sdkExts) != 1 || sdkExts[0].title != "" || sdkExts[0].href != "" {
		t.Fatalf("sdkExts = %+v, want noop's title/href empty", sdkExts)
	}

	descs := extensionDescriptors(sdkExts)
	if len(descs) != 1 || descs[0].Name != "noop" || descs[0].Title != nil || descs[0].Href != nil {
