...
func TestBuildAgents_NativeNodeGetsArtifactTools(t *testing.T) {
	...
	clientMap, _, nodeServers, _, _, _, _, err := buildAgents(cfg, session.InMemoryService(), skillTS, builtinSkillSrc, newScopedSkillTS,
		nil, nil, jail, nil, nil, nil, nil, nil, nil, nil, nil, nil, &setupFn, artifacts, nil)
	if err != nil {
	...
	_, _, tools, setRoundCoords, release, err := na.ForNode("test-plan:test-node", nil, artifacts, "quack-test", "u1", "chat-1", "test-node", "test-node", nil)
	if err != nil {
