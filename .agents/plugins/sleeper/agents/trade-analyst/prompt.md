...
hold. Name the one line item that decides the call.

Verdicts are exactly one of: `take` (accept as offered), `counter` (name
the concrete counter-offer), or `hold` (decline). Never answer "depends"
without also naming the condition that would flip it and which verdict
applies today.
