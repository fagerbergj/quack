...
- **v11.17.0** moved C4 element rendering onto the unified shape system (using the new `person` shape internally for `Person`/`Person_Ext`). This is an internal rendering change, not a syntax change — existing diagrams are unaffected. It did regress label wrapping: element labels stopped wrapping and could overflow `c4.width`. (History: pre-11.17 the legacy renderer wrapped by default via `c4.wrap`; 11.17.0 regressed to unwrapped and only the root-level `wrap: true` re-enabled it, since `c4.wrap` was ignored in that release; **11.17.1** wraps by default again and gates on `c4.wrap`, default `true`.)
