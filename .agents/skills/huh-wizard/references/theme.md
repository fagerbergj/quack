...
`internal/wizard/theme.go`. Build once, route every form through `runForm`. Start from `ThemeBase()` so
layout/spacing stay conventional and only the accent colors move.
...
Every form goes through this so the theme is consistent and you can't forget it. It also forces an
alt-screen: huh renders inline by default, so a tall group (a long `/models` list) scrolls the section
title off the top of the terminal. `WithProgramOptions` *replaces* huh's defaults, so re-supply the
stderr output (keeps stdout pipeable) and focus reporting.
...
- A custom theme is taste + risk: the ANSI escapes leak into golden snapshots, so don't theme a surface
  you golden-test without accounting for it. Keep the default theme if there's no brand call.
- Blurred styling matters as much as focused: muting unfocused fields is what makes the one focused field
  read as the subject. It's also what lets you stack several selects in one group (see `forms.md` →
  specialist models) — blurred selects collapse to just their value.
