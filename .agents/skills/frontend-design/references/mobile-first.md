...
2. **Hidden nav costs completion.** NN/g measured hamburger-hidden-nav rated 15% harder and completing 21% fewer tasks than visible nav; hidden menus scored worst of tested patterns. ([nngroup.com](https://www.nngroup.com/articles/hamburger-menus/))
   Quack: below `sm:` (640px), the persistent `NavRail` (`frontend/src/components/NavRail.tsx`, fixed `w-40`) becomes an off-canvas drawer with a visible hamburger entry point - not permanently hidden, not a narrower rail. **No bottom tab bar** - two top-level destinations (Chats, Memory) plus a chat-list drawer is NN/g's "combo" pattern (visible entry point + menu), which a footer nav doesn't fit.
...
4. **Responsive layout / container queries.** MDN's responsive-design guidance plus Tailwind v4's native `@container` support. ([developer.mozilla.org](https://developer.mozilla.org/en-US/docs/Web/Guide/Responsive_web_design), [tailwindcss.com](https://tailwindcss.com/docs/responsive-design))
...
- **#1133 - persistent nav rail.** `NavRail` was a fixed `w-40` column on every viewport (44% of a 360px screen), squeezing the composer to 55px wide. Fix: `NavRail` gets a compact-width variant that renders off-canvas (`fixed inset-y-0 -translate-x-full` sliding in over content, reusing the existing chat-list drawer's slide-in pattern) below `sm:`, with a hamburger button replacing the always-visible rail. Above `sm:`, keep the persistent rail. This is the highest-impact fix - it also reduces the severity of #1134 and #1136 downstream.
...
- **#1135 - artifacts dialog stays a small floating panel.** ~200px wide, off-center, truncated header, tiny close button. Fix: full-width/full-height bottom sheet below `sm:` per rule 6 above; audit other dialogs (rename, filters) for the same fixed-desktop-size assumption while in there.
...
5. Test RTL: force a Tailwind logical-property mismatch by checking `dir="rtl"` renders correctly, and verify any `matchMedia`-driven breakpoint logic (if a component reads viewport width in JS instead of pure CSS) fires the same in RTL.
