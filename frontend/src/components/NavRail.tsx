...
export interface NavRailProps {
  ...
  // Storybook seam: forces the compact (<600px) branch regardless of the
  // real window width, so a story can show the hamburger/drawer
  // deterministically without an addon-viewport dependency this repo
  // doesn't have. Tests still drive this via matchMedia (see NavRail.rtl.test.tsx).
...
export function NavRail({ route, activeExtension, initialCollapsed, initialExtensions, forceCompact }: NavRailProps) {
  ...
  const mediaCompact = useCompact()
  const compact = forceCompact ?? mediaCompact
  ...
  const drawerPanelRef = useDrawer(drawerOpen, () => setDrawerOpen(false))

  // Compact (<600px, #1131/#1133): the persistent rail is replaced entirely by a
  // 44px hamburger - a visible, always-reachable entry point (NN/g's "combo"
  // pattern, never a hidden-only menu) that opens an off-canvas drawer holding
  // the same Chats/Memory/extensions list as the expanded rail, instead of a
  // rail that merely narrows and still eats a quarter of a 360px screen.
  if (compact) {
    return (
      <>
        <nav aria-label="Main" className="flex-shrink-0 h-full flex flex-col items-center border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-2">
          <button
            ...
            className="flex items-center justify-center w-11 h-11 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
          >
          ...
          <div className="fixed inset-0 z-40">
            ...
            >
              ...
              <div className="flex-1 py-2 px-2 space-y-1 overflow-y-auto">
                <NavItem icon="💬" label="Chats" active={route === 'chat'} onClick={() => { navigate('/chat'); setDrawerOpen(false) }} />
                <NavItem icon="🧠" label="Memory" active={route === 'memory'} onClick={() => { navigate('/memory'); setDrawerOpen(false) }} />
                {linkedExtensions.length > 0 && (
                  <div className="pt-1 mt-1 border-t border-gray-100 dark:border-gray-700 space-y-1">
                    {linkedExtensions.map(ext => (
                      <ExtensionNavItem key={ext.name} ext={ext} active={activeExtension === ext.name} onNavigate={() => setDrawerOpen(false)} />
                    ))}
                  </div>
                )}
              </div>
