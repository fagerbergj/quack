 ...
 * pair tools by `call_id`. Every tool call surfaces as ordinary tool activity
 * (`agent_tool_call` / `agent_tool_result`) within the worker's own run - never
 * a separate stage.
 *
 ...
 *
 * A direct per-id lookup (the same store.GetByID deleteMemory/voteMemory
 * already use), not a List scan - correct and O(1) regardless of corpus
 * size. Includes an invalidated memory (status: invalidated) rather than
 * 404ing, same as include_invalidated=true on the list endpoint.
 *
 */
 ...
 * the nightly consolidation sweep does - same code path, so this is
 * also how an operator forces the nightly sweep's forgetting step to
 * run early. Never removes a point (retention, not this endpoint, does
 * that after the configured retention window).
 *
 */
