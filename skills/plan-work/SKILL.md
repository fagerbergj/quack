...
**Every reviewer node in a fan-out posts its own findings** (`stage_review_comment`/`stage_review`) scoped to its slice - #948's fan-in merges them (worst-of verdict), so no node needs to see the others' output or wait to validate pooled findings. Its task names its slice AND says its verdict is scoped to the slice, not the whole PR: "Review these files in PR #N: `<paths>`. Check out the PR head branch, read their diff plus enough surrounding code to judge correctness, and stage findings + a verdict for your slice. Do not stray outside your slice. Your verdict covers ONLY your slice - stage `approve` if your slice has no blocking findings, even though you haven't reviewed the rest of the PR; a run-scoped merge combines every slice's verdict into the PR's actual verdict, so a clean slice must not default to `comment`."
