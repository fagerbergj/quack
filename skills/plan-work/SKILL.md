...
**Every reviewer node in a fan-out posts its own findings** (`stage_review_comment`/`stage_review`) scoped to its slice - #948's fan-in merges them, so no node needs to see the others' output or wait to validate pooled findings. Its task names its slice: "Review these files in PR #N: `<paths>`. Check out the PR head branch, read their diff plus enough surrounding code to judge correctness, and stage findings + a verdict for your slice. Do not stray outside your slice."
