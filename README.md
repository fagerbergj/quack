...
5. **Memory.** Quack writes durable memories — vetted findings, user preferences, and prior
   ...
   avoids redoing work. Only adversarially-vetted output is committed to memory. (Its own
   `memory.Store`, not ADK's automatic session memory — see **Memory** below.)
...
gateway, and from there the **orchestrator** plans a DAG of agents that runs as one native ADK
graph, every node wrapped in quack's own **trust gate** before its output counts. It all sits on
pluggable model providers and data stores. Runs happen in the background, stream their progress,
...
providers, and per-agent model choices live in [docs/configuration/index.md](docs/configuration/index.md).
...
output passes a **trust gate** (`vetting.RunGatedRefine`), cheapest-first:

1. **Continuation (free).** Mechanical completion signals — an empty answer, an undelivered
   commit, an unposted review — hand the worker another tool-bearing round, bounded.
2. **Deterministic checks (free).** Citation backing, length, delivery/review criteria, and the
   repo's own build/vet/test commands, run against the actual clone. No model call needed to
   catch these.
3. **Independent judge.** A separate, independently-configured judge model (e.g.
   [`gemma4-26b-a4b`][selenemini]) scores the output G-Eval style, weakest-link (the lowest
   criterion decides, no averaging). Because it is genuinely different weights from the worker, it
   catches blind spots the worker cannot see in its own output. This is the real trust decision
   ([the case for adversarial agents][adv]).
...
**How it maps to ADK.** Nodes are not ADK's stock [LoopAgent critic-refiner pattern][adk-loop] —
the gate's continuation/checks/judge stages don't fit that shape, and code agents run as external
subprocesses with no ADK sub-agent to loop. Instead the whole plan runs as **one native ADK graph**
(`internal/dag/nativegraph.go`): nodes are workflow nodes wired by dependency edges, and each is
wrapped by `vetting.RunGatedRefine` before its output flows to dependents. Per-agent models are
just different `model.LLM`s bound to the worker and the judge.
...
2. **Execute.** The DAG runs as **one native ADK graph** under a single runner (leaves fan out
   from Start, a node with multiple dependencies waits on a join barrier), passing outputs along
   edges with limited concurrency, per the inference constraint.
3. **Vet (auto-wrapped nodes).** Every node is wrapped in the **trust gate**
   (`vetting.RunGatedRefine`): continuation → deterministic checks → independent judge, bounded
   rounds at each stage. The gate lives *inside* the node, never as a cycle in the graph, so the
   DAG stays acyclic.
...
trust gate after max rounds is retried, then aborts the branch / escalates to the user with
what was learned.
...
A fourth client sits in front of these over its own webhook, not a protocol face: the
**GitHub App** ([`docs/extensions/github.md`](docs/extensions/github.md)). It drives runs from a repo directly — the
`quack:plan` / `quack:implement` / `quack:review` / `quack:merge` label workflow, and
`@quack` mentions on issues/PRs — and replies on the issue/PR when the run completes.
...
**Memory** is quack's own `memory.Store` backed by the vector store (semantic) + the relational
store (structured); per Goal 5, **only a gate pass commits a task finding** (user facts are
...
The full shape — providers, gates, tools, agents, budget, auth, stores — plus the actual model
assignments, thresholds, and env vars (with justifications) live in
[docs/configuration/](docs/configuration/), kept separate so they can change without touching this
document.
...
orchestrator discovers agents by their cards and dispatches nodes to them; each node runs through
the trust gate like any other.

The code agents (code-implementer, code-reviewer, code-explorer) are the exception to "bound to a
model and a set of tools": they run as **external ACP subprocesses** (`opencode acp`) with no
...
- [`docs/`](docs/) — human-written setup and configuration guides:
  [`docs/configuration/`](docs/configuration/), [`docs/extensions/github.md`](docs/extensions/github.md), [`docs/faq.md`](docs/faq.md).
- [`openwiki/`](openwiki/) — an auto-generated code reference (architecture, workflows,
  operations), refreshed from the code. Good for "what the code does."
- [`CONTRIBUTING.md`](CONTRIBUTING.md) — how to contribute: the issue → plan → implement →
  review → merge loop.
