...
A GitHub webhook run enters differently: `internal/github/webhook.go` handles its own route (mounted by `router.go` as an extension, outside the OpenAPI contract) and calls `Orchestrator.Run` directly, bypassing `rest/` - see "GitHub extension" under Trust gate below.
