...
| `agents` | no | Agent bundle names to seed - the source of truth over directory presence. Each must exist at `agents/<name>/`, or boot refuses the plugin naming the entry; a bundle present but absent from this list is skipped (warned), not seeded. Omitted entirely: bundles are discovered by presence instead. |
...
`internal/plugin.Resolve` sets `AgentsDir`/`WorkflowsDir` on a resolved `Plugin` when those directories exist, the same way `skills/` is found. The namespace block's `agents`/`workflows` lists are the contract for what actually seeds: listed and present seeds, listed and missing refuses the plugin, present but unlisted only warns. A plugin that omits both lists falls back to seeding everything present, same as before this contract existed. Two plugins listing the same agent or workflow name is also a boot refusal, naming both - manifest lists never silently merge. A subdirectory of `agents/` is a bundle only if it has an `agent-card.json`; every `*.yaml` file under `workflows/` is one shape.
