...
- Booting into a replay never fetches or writes the plugin registry: seeding and each row's `git fetch` both skip, so a replay makes no live git calls and never mutates a registry a real server could be running against at the same time - each plugin is still pinned by sha from its clone's existing history.
