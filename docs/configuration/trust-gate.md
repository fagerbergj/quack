...
Against a server, replay also runs the shadow locate tier from [`docs/design/judging.md`](../design/judging.md) beside the judge: code segments the answer into units (sentences, list items, table rows), finds each unit's specifics (figures, percentages, currency, dates, quoted strings, proper names) and pairs the unit with its citation (inline, else the nearest following one in its block, `[n]` markers resolved through the reference list); each specific is then looked for in the cited page's stored text. The text report prints a tally per round - `located`, `unlocated`, `uncited`, `no_stored_text` - and only the rows that did not locate; `--json` carries every row with its evidence window. Located never means supported: this tier can reject or locate. `--verify` runs the verify tier over the located rows: the judge model, with no tools, reads each item's evidence window as data and answers supported, unsupported or cannot_tell with a verbatim quote; code drops any verdict whose quote is not in the window or, for a figure, does not contain it (`not_checked` - a failed verifier is never a negative verdict). Items are batched per cited page, one call per page. Nothing here gates yet.
