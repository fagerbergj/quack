...
Replay exits non-zero the moment any criterion flips.
...
- Recorded scores are grouped by each `eval.score` entry's `ResponseID`, keeping only the latest run that judged a given (node, round) label - a queued re-run or a reused node id can otherwise record two distinct runs' criteria under the same label. The task text is not deduplicated the same way: for a re-run sharing a node id, it comes from whichever round ran earliest and may lack that later run's own queued-message section.
