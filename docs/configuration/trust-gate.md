...
Pass/fail is decided by `gates.judge.threshold` from the local `quack.yaml` - the same bar `buildEnvelope` gates on live - for both the recorded and the replayed score. Each criterion's own `scale.pass` from the rubric is printed alongside as `rubric_mark`, informational only: the live gate does not read it. A criterion the live gate recorded but replay could not reproduce (an edited rubric dropped it, or its inputs couldn't be rebuilt) is its own flip class, not silently omitted. Replay exits non-zero the moment any criterion's pass/fail flips.
...
- The judge's read access: with `--from-server`, the judge gets `list_artifacts`/`read_artifact` over that server's REST API (real access, not the recording); a local bundle file has no server to read from, so a round whose activity wrote an artifact is replayed deterministic-only with a note, since the live judge prompt always advertises those tools.
