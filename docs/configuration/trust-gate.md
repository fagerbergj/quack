...
It rebuilds, per judged round: the node's own task (the built prompt actually sent to the worker, which live also uses as the judge's question) and the answer that round graded, from whichever of the node's own rounds chronologically precedes it - not assumed from the round-id string, since a hitl/confirm/continuation/revise/queued-retry round takes several different id forms. Worker activity (fetched/seen URLs, workspace ops, data-tool call/results) is rebuilt from every stream in the WHOLE recorded chat session (not just the judged node's own), replayed through a fresh in-memory session and scanned with the gate's own `activityFromSessionAt` - matching the live gate's `ctx.Session()` scan, which is chat-scoped, not node-scoped. `--rubric <path>` overrides the graded node's own bundled `rubric.yaml`, and its rendered text reaches `cfg.Rubric`, so an edited criterion definition actually reaches the judge prompt.

`--repeat N` judges each round N times over the same rebuilt answer and prints, per judge-scored criterion, the samples' mean, standard deviation and pass rate at the gate threshold. Same answer, same rubric, so the spread is the judge model's own noise; a criterion whose pass rate sits between 0 and 1 is one a single verdict cannot be trusted on, and the score distribution is what a per-criterion pass mark should be chosen from. Deterministic criteria are re-run on every pass, but only the first pass's score is reported; a pass that fails mid-measurement is noted on the round and exits 1.

Against a server, replay also runs the shadow locate tier from [`docs/design/judging.md`](../design/judging.md) beside the judge: code segments the answer into units (sentences, list items, table rows), finds each unit's specifics (figures, percentages, currency, dates, quoted strings) and pairs the unit with its citation (inline, else the nearest following one in its block, `[n]` markers resolved through the reference list); each specific is then looked for in the cited page's stored text. The text report prints a tally per round - `located`, `unlocated`, `uncited`, `no_stored_text` - and only the rows that did not locate; `--json` carries every row with its evidence window. Located never means supported: this tier can reject or locate. `--verify` runs the verify tier over the located rows: the judge model, with no tools, reads each item's evidence window as data and answers supported, unsupported or cannot_tell with a verbatim quote; code drops any verdict whose quote is not in the window or, for a figure, does not contain it (`not_checked` - a failed verifier is never a negative verdict). Items are batched per cited page, one call per page. An unlocated figure gets a second look: the window is cut around the claim's own key terms in the page and the verifier reads that instead, so a page that states the figure differently (or spells it out) can still settle it. Nothing here gates yet.
