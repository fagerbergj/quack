...
- `rubric.yaml` (optional) — a per-agent judge rubric, overriding `config/rubric.md`.
  ...
  A judged criterion names what one finding is and asks the judge for a verbatim quote or a named omission per finding; a finding it cannot quote does not count. Keep each criterion to questions the judge can check by looking (a citation in the same sentence, an outlet name matching the linked host, two passages that cannot both be true) and leave anything code already computes out of it - `cites_sources` is code-owned, and a judge score for any `deterministic: true` criterion is dropped. Measured on 17 recorded web-researcher and synthesizer rounds with the 27B judge (`quack judge replay <chat> --repeat 3`): this wording took `clean_output` from a 0.14 swing with 4 pass/fail flips to 0.03 and none, while `grounded`, `citation_quality` and `internally_consistent` stayed at their baseline swing (0.10-0.15) because the judge finds a real defect on some passes and not others - wording does not fix recall. Counting quoted findings does not by itself steady those three: the first counted version, with broad finding definitions, doubled their flips, and narrowing what a finding is only brought them back to baseline. Prove a rubric change with the same replay before shipping it. Citation presence is code-owned: a rubric that declares `specifics_cited` (`deterministic: true`) gets the share of its substantive specifics (figures, percentages, prices, dates) that carry a citation in their own sentence or bullet, or the next one after them in the same paragraph, or appear in the research the node received, computed over the answer plus the artifacts the node wrote this round, so a pointer answer is scored on its artifact and a fan-in synthesizer inherits its dependencies' citations; the judge's `grounded` then asks only whether the answer contradicts its own specifics. A rubric that also declares `specifics_supported` gets the page check: each cited specific is looked up in its cited page's stored text (or, when the page was never fetched, in the search snippet the worker saw for it, which can confirm a specific but never contradict one, since a snippet is partial and often stale) and read by a tool-less model call that must quote the page (code rejects a quote that is not there); a contradiction is read once more in a window three times wider, and one that holds on both reads fails the round. It runs beside the judge inside the judge's admission slot, so it adds no wall time while the judge is the slower of the two; a round whose judge call fails does not wait for it, and it is absent when nothing could be read.
