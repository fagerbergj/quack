...
- `allow_clone` — set on `code-explorer` only: lifts the `git clone`/`gh repo clone` deny (and the cwd-only `external_directory` boundary) so it can shallow-clone a third-party repo into `$TMPDIR` and read it locally. Requires `read_only`; config validation rejects it otherwise. `git push` stays denied for every agent.
