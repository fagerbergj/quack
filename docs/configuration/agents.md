...
- `rubric.yaml` (optional) — a per-agent judge rubric, overriding `config/rubric.md`.
  ...
  A judged criterion is scored by counting quoted findings, never by weighing adjectives: its first step tells the judge to enumerate the items it covers and to record each failure with a verbatim quote or a named omission (a finding that cannot be quoted does not count), and its four bands are always the same shape - 3 no finding, 2 exactly one minor finding, 1 two or three findings or one on a load-bearing item, 0 four or more or the criterion's central failure - with the criterion's own words for what a finding is and what makes one minor, load-bearing or central. Words like "essentially every", "most", "a few" or "a touch more confidently" do not belong in a band: the 27B judge stepped between adjacent bands on one call in three when they were separated by adjectives. Prove a rubric change with `quack judge replay <chat> --repeat 3` over recorded rounds before shipping it.
