...
`prompts:` (see [index.md](index.md)) points named artifacts at a `stores:` entry of `kind: langfuse`; a round whose prompt resolved from a store gets its generation spans stamped `langfuse.observation.prompt.name`/`langfuse.observation.prompt.version` (Langfuse v4's documented prompt-link keys), the pair Langfuse uses to link a generation to the prompt version that produced it - the name is the resolved artifact (`system/<agent>` or `system/judge`), not the agent's config key. Every span already carries `gen_ai.conversation.id` (the chat id) and `user.id` (the session's user, when known), so a Langfuse trace is filterable by session/user without extra wiring. A node's own span additionally gets `langfuse.observation.output`/`langfuse.trace.output` set to the delivered text once a delivery commits (gated on `observability.otel.capture_content` like every other content span attribute, off by default), so the trace's result is the thing that actually shipped, not any one round's draft.
...
`quack replay` pins each recorded `llm.call` to the exact prompt version it ran on, using that entry's `prompt_source`/`prompt_version_id`/`prompt_artifact` (#1422): a `static` entry replays only if the shipped file on disk still hashes to the recorded id; a store-sourced entry (`langfuse`, say) is fetched by that exact version number from the store named in the replaying deployment's `prompts.store` - and refuses if that store's name doesn't match the one the run was recorded from (version numbers are per Langfuse project). Any of these failing - a drifted shipped file, a deleted store version, a mismatched store, or no `prompts.store` configured at all - refuses the whole replay up front, naming `<name>@<version>`, rather than silently falling back to latest or static. A name that resolved to more than one version across the recording (rounds re-resolve their prompt by design) also refuses, naming both versions, rather than guessing which round to pin. An entry recorded before #1437 (no `prompt_source`) has no version to pin, so it replays as before: a live/static resolve with `prompt_version` compared only as an informational drift warning.

Only `system/<agent>` (the worker's and judge's own system prompt) carries this per-call provenance today; `rubric/*`, `memory/*`, `system/compaction*` and `system/acp.environment` resolve live/static during replay same as any other artifact, unpinned - a known gap, not by design.
