...
`system/<agent>` (the worker's and judge's own system prompt) carries per-call prompt provenance (`prompt_source`/`prompt_version_id`/`prompt_artifact`, #1422) as before. Each gated round's `llm.call` (native) or `agent.invoke` (ACP) now also carries the full `artifacts: [{name, source, version_id}]` list of every artifact that round actually resolved through `artifactsrc` - a worker round's own `system/<agent>` plus `memory/<agent>` when the bundle has one, a judge round's `system/judge` plus `rubric/global`/`rubric/<agent>` and `rubric/constitution`, and an ACP round's `system/acp.environment` plus its preamble's `system/<agent>` (#1455). The same entry carries `plugins: [{name, sha}]` for every plugin registry row in scope, on native rounds as well as ACP rounds; a registry row's `sha` is its pinned commit, the embedded `quack` plugin and local roots omit it. `system/compaction*` is resolved once at node build, off any round's context, and is not yet attributed to a specific round's entry - still a known gap. `quack dataset export` copies both lists into the exported item's metadata.
