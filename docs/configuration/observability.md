...
Only `system/<agent>` (the worker's and judge's own system prompt) carries per-call prompt provenance (`prompt_source`/`prompt_version_id`/`prompt_artifact`, #1422) today; `rubric/*`, `memory/*`, `system/compaction*` and `system/acp.environment` resolve live/static like any other artifact, unpinned - a known gap, not by design.
