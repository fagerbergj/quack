...
`quack replay` pins each recorded `llm.call` to the exact prompt version it ran on, using that entry's `prompt_source`/`prompt_version_id` (#1422): a `static` entry replays only if the shipped file on disk still hashes to the recorded id; a store-sourced entry (`langfuse`, say) is fetched by that exact version number from the store named in the replaying deployment's `prompts.store`. Any of these failing - a drifted shipped file, a deleted store version, or no `prompts.store` configured at all - refuses the whole replay up front, naming `<name>@<version>`, rather than silently falling back to latest or static. An entry recorded before #1437 (no `prompt_source`) has no version to pin, so it replays as before: a live/static resolve with `prompt_version` compared only as an informational drift warning.
