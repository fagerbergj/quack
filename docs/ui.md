...
- Manage the dynamic plugin registry on the Plugins page: name, source, ref, installed sha, fetched-at, and any error per row; per-row and all-behind Update buttons; add a `github:` entry or remove a `github:` row (see [`agent-plugins.md`](agent-plugins.md#the-registry)).
