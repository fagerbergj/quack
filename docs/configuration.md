...
| `tools.<store-backed>.store` | a `stores[]` name | A tool backed by *shared* infra (memory, documents) references a store instead of `kind`+`url`; the store supplies the adapter + connection. May override `collection` / `schema` / `top_k` / `min_score`. |
...
| `stores.<name>.kind` | `postgres` / `qdrant` / `opensearch` | **Named backend registry** (like `providers`): consumers reference a store by name. `kind` selects the adapter (the portability seam). Implemented: postgres, qdrant; opensearch reserved for M8. |
...
| `stores.<name>.extends` | another store name | Inherit a store's fields (child overrides) — e.g. the `document` store reuses `default_postgres`'s connection. |
