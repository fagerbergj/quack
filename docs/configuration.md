...
| `agents[code-implementer].tools` | `read_file, write_file, edit_file, list_dir, glob, grep, delete_path, git_clone, git_status, git_diff, git_log, git_commit, git_branch, git_worktree_create, git_worktree_remove, git_push, git_pull, git_rebase, run_command, ask_advisor, ask_user` | The full fs + git surface, plus `run_command` for its own build/test iteration loop (guarded — see `workspace.guards`). |
...
| `workspace.timeout_seconds` | `60` | Per-invocation timeout for git commands, `run_command`, and orchestrator-set deterministic checks — all three execute through the ONE jailed argv runner (`internal/workspace.RunArgv`). |
| `workspace.check_commands` | `["go build", "go test", "go vet", "npx tsc", "npm test"]` (empty ⇒ checks unavailable) | Allowed command **prefixes** the planner may complete into a code-implementer node's per-node `checks` (§4 of `.quack/plan-pr5-tool-schemas.md`). The planner rejects a plan whose `checks` don't prefix-match one of these (or contain a shell metacharacter) at PLAN time, not run time; when this list is empty the planner is told checks are unavailable and omits them. The trust gate's deterministic stage then runs each configured check via `RunArgv` in the node's `workdir` after every draft, folding a failure into criterion `checks_pass = 0` (weakest-link, like `grounded_in_retrieval`) — command + output tail become part of the revise prompt's feedback. |
