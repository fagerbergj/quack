...
An ACP-agent fork replay refuses to spawn its live subprocess when a recorded plugin's sha differs from the one currently installed, or the plugin is no longer registered - a live fork spawn always reads the current clone, so it cannot reproduce the recorded skill text once the plugin has moved. To fork-replay against the plugin state as recorded, re-pin the plugin to that sha before starting the replay: `POST /api/v1/plugins` with `github:owner/repo@<sha>` (or edit its `plugins.seed` entry and restart), which checks the clone out at that sha and records it as the installed sha.
