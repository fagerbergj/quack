...
The full evidence the envelope only summarizes or caps - the raw webhook payload, the untruncated comment thread, the issue timeline, and (on a CI-triggered run) `check-runs`/`annotations-<check>` - is stored as named input artifacts, not files in a directory (issue #1010 deleted the context-dir mechanism). The envelope's `<artifacts>` manifest lists each one's id, revision, and new/unchanged status against the last dispatch (e.g. `<artifact id="comments" revision="4" status="new">47 total, 3 new</artifact>`); an agent reads the untruncated content with `read_artifact("bytes:<name>")` - e.g. `read_artifact("bytes:comments")`, `read_artifact("bytes:check-runs")`, `read_artifact("bytes:annotations-go-test")` - the `bytes:` prefix is the recordstore kind these are all saved under, not part of the manifest's own `id` attribute.
