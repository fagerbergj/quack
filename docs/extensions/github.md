...
The full evidence the envelope only summarizes or caps - the raw webhook payload, the untruncated comment thread, the issue timeline, and (on a CI-triggered run) `check-runs`/`annotations-<check>` - is stored as named input artifacts, not files in a directory (issue #1010 deleted the context-dir mechanism). The envelope's `<artifacts>` manifest lists each one's local name, revision, and new/unchanged status against the last dispatch (e.g. `<artifact id="comments" revision="4" status="new">47 total, 3 new</artifact>` - the pinned github v0.8.1 renders the bare name here, not the store's own id). `read_artifact` takes the store's id, which is always `bytes:<name>` (e.g. `read_artifact("bytes:comments")`) - prefix the manifest's `id` with `bytes:` before calling it. A later extension release is expected to render the full id directly; once the pin bumps to it, this prefixing step goes away.
