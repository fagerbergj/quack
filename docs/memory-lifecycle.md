...
**(c) Background consolidation pass.** New, separate from the per-commit reconcile: a periodic job (`stores.default_vector.consolidation.interval`, 0 = disabled, matching `ledger.RunRetentionSweep`'s `retentionDays <= 0` convention) that, per bucket, clusters memories by `chat_id` + temporal proximity and calls the same `decide()`/`apply()` machinery with a dedupe-flavored prompt variant — collapsing the four #849 variants into one `ADD` + three `DELETE`(→invalidate, reason `"duplicate of <id>"`) had outcome feedback not already invalidated them first. Reuses the existing op taxonomy; only the trigger (cron, not commit) and candidate set (existing unverified memories, not this run's staged candidates) differ.
...
**The consolidator job**: new `internal/memory/consolidate_job.go`, started next to `ledger.RunRetentionSweep`'s call site in server bootstrap. `Store.RunConsolidationSweep(ctx, interval)` — ticker loop, same shape as the ledger sweep, same nil/zero-disables convention.
...
5. **Periodic consolidator job** (burst-dedupe) + config `interval` + retention sweep. ~1–2 PRs.
