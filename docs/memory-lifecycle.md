...
**Usage tracking.** Every recall delivery that routes through `LogRecall` - the ACP
prefill injection in `vetting/node.go`, `recall_memory`/`load_memory` (native workers and the orchestrator - `load_memory` is an alias onto the same tool, so it is logged and votable identically; the ACP loopback MCP's own `load_memory` is a separate, unlogged prose-recall tool - only its `recall_memory` is recorded), and the plan judge's project-memory injection (source `plan_judge`) - appends a `memory.recall` ledger entry (chat, node, round, source, delivered ids+scores) and bumps `recalls`/`last_recalled_at` on the point (one batched write, unconditionally - a missing ledger only skips the ledger append). A native agent's automatic ADK `preload_memory` injection is NOT recorded: it delivers no citable id for the judge to vote on, so counting it would feed the recalls-based forgetting rule below from an unvotable delivery. The ledger is the source of truth for what a chat retrieved - unlike a vote, a recall never writes a `memory_ops` row; the audit trail for retrieval lives entirely in the ledger (`internal/ledger`'s `KindMemoryRecall`), and `internal/ledger/fold` folds it into per-id recall counts so `quack ledger rebuild` can re-derive the same projection from scratch.
