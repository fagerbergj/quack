...
artifacts recorded. All six checked-in GET fixtures cover `GET /app` (bot identity), the fixture issue (`/repos/fagerbergj/quack-qa/issues/1`), the fixture PR (`/repos/fagerbergj/quack-qa/pulls/1`), the issue comment list (`/repos/fagerbergj/quack-qa/issues/1/comments?per_page=100`), and the two installation lookups (`/repos/fagerbergj/quack-qa/installation` plus `/repos/fagerbergj/quack/installation` for the delivery path's real owner/repo); `pulls/1/{files,commits,comments,reviews}`, `issues/1/comments`, and `commits/<sha>/check-runs` still 404 - harmless, since every one of those fetches is best-effort and the extension
